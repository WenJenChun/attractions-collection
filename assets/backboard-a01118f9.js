import"./main-1da5e8c0.js";const r=localStorage.getItem("token")!==null,s=localStorage.getItem("role")==="admin",d=document.getElementById("header-content");d.innerHTML=`
<%- include('./layout/header.ejs', { isLogIn: ${r}, isAdmin: ${s} }); -%>
`;const u=document.querySelector("#attractionList"),c="https://attractions-api-jhwt.onrender.com",l=document.querySelector("#navBar");function m(){axios.get(c+"/views").then(function(e){console.log(e.data);const i=e.data;let o="";i.forEach(function(t){o+=`
        <tr>
        ...
        </tr>`}),u.innerHTML=o,document.querySelectorAll(".deleteAttraction").forEach(function(t){t.addEventListener("click",function(){const n=t.getAttribute("data-attraction-id");console.log("按"+n),axios.delete(`${c}/views/${n}`).then(function(a){alert("刪除成功！"),location.reload()}).catch(function(a){console.error("刪除失敗："+a)})})})})}m();localStorage.getItem("token")==null?console.log("還沒登入"):(console.log("已登入"),localStorage.getItem("role")=="admin"?l.innerHTML=` 
